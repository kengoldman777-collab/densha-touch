...
function makePath(y){const p=[{x:-500,y},{x:1500,y},{x:1500,y:-900-y},{x:-500,y:-900-y},{x:-500,y}],cum=[0];for(let i=1;i<p.length;i++)cum.push(cum[i-1]+Math.hypot(p[i].x-p[i-1].x,p[i].y-p[i-1].y));return {p,cum,length:cum.at(-1)};}
...
reset(){this.paths=[makePath(370),makePath(650)];this.trains=[];this.crossings=this.paths.map((p,track)=>({track,d:1000,y:p.p[0].y,requested:false,closed:false}));this.cars=[{y:50,dir:1,color:'#ed986a'},{y:940,dir:-1,color:'#70a5c2'},{y:-200,dir:1,color:'#e8c85c'},{y:1190,dir:-1,color:'#bd9acb'}];}
...
// The invisible return journey is skipped only after every car has left the screen.
if(t.d>2200){t.d=150;t.traveled=0;}}
